      ...
      <AnimatePresence mode="wait">
        {view === 'form' && !submitted && (
          <motion.div key="form" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="space-y-4">
            <div className="glass-card-glow p-5 space-y-4">
              ...
              <div>
                <label className="text-sm font-medium flex items-center gap-1 mb-1"><Package className="w-3.5 h-3.5" /> Waste Type</label>
              ...
              <div>
                <label className="text-sm font-medium mb-1 block">Approx. Weight (kg)</label>
                <Input type="number" placeholder="e.g. 5" value={weight} onChange={e => setWeight(e.target.value)} min="0.5" step="0.5" />
              </div>
              <div>
                <label className="text-sm font-medium flex items-center gap-1 mb-1"><Clock className="w-3.5 h-3.5" /> Preferred Time</label>
        ...
        {submitted && (
          <motion.div key="submitted" initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} className="text-center py-10">
            <motion.div className="w-16 h-16 mx-auto eco-gradient rounded-full flex items-center justify-center" initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ type: 'spring' }}>
              <Check className="w-8 h-8 text-primary-foreground" />
            </motion.div>
            <h2 className="text-xl font-bold mt-4">Pickup Scheduled! 🚚</h2>
              ...
              user.pickups.map(p => (
                <div key={p.id} className="glass-card p-4 space-y-3">
                  <div className="flex justify-between items-start">
                    <div>
                      <p className="font-semibold">{p.wasteType} — {p.weight} kg</p>
                      <p className="text-xs text-muted-foreground">{p.address}</p>
                      <p className="text-xs text-muted-foreground">{p.timeSlot} · {p.date}</p>
                      {p.agent && <p className="text-xs mt-1">Agent: <span className="font-medium">{p.agent}</span></p>}
                    </div>
                  ...
                  <div className="flex items-center justify-between">
                    <span className="text-xs font-medium text-primary">{statusLabels[p.status]}</span>
                    {p.status !== 'collected' && (
